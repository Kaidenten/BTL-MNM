...
<div class="card border-0 shadow-sm">
  ...
  <?php if ($tx['receiver_name']): ?>
    <span><i class="bi bi-wallet2 me-1 text-primary"></i><?= htmlspecialchars($tx['receiver_name']) ?></span>
  <?php elseif (!empty($tx['bank_info'])): ?>
    <span title="<?= htmlspecialchars($tx['bank_info']) ?>">
  ...
  <?php endif; ?>
          ...
          <td>
            <button class="btn btn-sm btn-outline-secondary"
                    ...
                    data-bankinfo="<?= htmlspecialchars($tx['bank_info'] ?? '') ?>"
                    data-recv="<?= htmlspecialchars($tx['receiver_name'] ?? ($tx['bank_info'] ?? '—')) ?>">
              <i class="bi bi-eye"></i>
            </button>
          </td>
        </tr>
    ...
    <div class="modal-content border-0 shadow">
      ...
      <div class="modal-body px-4">
        <table class="table table-borderless small">
          ...
          <tr id="m-bankinfo-row"><td class="text-muted fw-semibold">Ngân hàng</td>   <td id="m-bankinfo"></td></tr>
...
document.getElementById('txModal').addEventListener('show.bs.modal', function (e) {
  ...
  const typeMap = {
    ...
    'withdraw': '<span class="badge bg-danger">Rút tiền</span>',
    'transfer': '<span class="badge bg-primary">Chuyển tiền</span>',
    'payment':  '<span class="badge bg-warning text-dark">Thanh toán</span>',
  };
  const statusMap = {
  ...
  document.getElementById('m-type').innerHTML     = typeMap[b.dataset.type]     || b.dataset.type;
  document.getElementById('m-status').innerHTML   = statusMap[b.dataset.status] || b.dataset.status;

  // Ẩn/hiện dòng ngân hàng
  const bankRow = document.getElementById('m-bankinfo-row');
  if (b.dataset.bankinfo) {
    document.getElementById('m-bankinfo').textContent = b.dataset.bankinfo;
