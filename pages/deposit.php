...
$db = getDB();
$stmt = $db->prepare("SELECT * FROM bank_accounts WHERE user_id = ? LIMIT 1");
...
document.querySelectorAll('.btn-quick-amount').forEach(btn => {
  btn.addEventListener('click', function () {
    ...
    document.getElementById('amount_display').value = Number(raw).toLocaleString('vi-VN');
    document.querySelectorAll('.btn-quick-amount').forEach(b => b.classList.remove('active','btn-primary'));
