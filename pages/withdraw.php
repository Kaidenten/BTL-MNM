...
$db = getDB();
$stmt = $db->prepare("SELECT * FROM bank_accounts WHERE user_id = ? AND is_verified = TRUE ORDER BY is_default DESC");
...
<div class="container">
  ...
  <div class="alert alert-warning small">
    <?php
    // Kiểm tra có TK nào đang chờ duyệt không
    $stmtPending = $db->prepare("SELECT COUNT(*) FROM bank_accounts WHERE user_id = ? AND is_verified = FALSE");
    $stmtPending->execute([$_SESSION['user_id']]);
    $hasPending = $stmtPending->fetchColumn();
    ?>
    <?php if ($hasPending > 0): ?>
      <i class="bi bi-hourglass-split me-1 text-warning"></i>
      Tài khoản ngân hàng của bạn đang <strong>chờ admin xác nhận</strong>.
      Vui lòng đợi sau khi được duyệt mới rút tiền được.
    <?php else: ?>
